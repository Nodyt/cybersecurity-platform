...
// Extend Request to include user info from middleware
...
export const getModuleDetails = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
...
export const completeModule = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        const module = await prisma.module.findUnique({ where: { id: moduleId as string } });
        if (!module) {
            res.status(404).json({ error: 'Module not found' });
            return;
        }
        ...
        let progress = await prisma.trainingProgress.findUnique({
            where: { userId_moduleId: { userId, moduleId: moduleId as string } }
        });
